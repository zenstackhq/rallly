...
const getPollIdFromAdminUrlId = async (urlId: string) => {
    const res = await db.poll.findUnique({
        select: {
            id: true,
        },
...
export const polls = router({
    ...
    getCountByStatus: privateProcedure.query(async ({ ctx }) => {
        const res = await db.poll.groupBy({
            by: ['status'],
            where: {
                userId: ctx.user.id,
                deletedAt: null,
        ...
        return res.reduce(
            ...
            {} as Record<ModelTypes.PollStatus, number>
        );
    }),
    /** @deprecated */
                ...
                ctx.analytics.trackEvent({
                    ...
                    properties: {
                        ...
                        requireParticipantEmail:
                            !!updatedPoll.requireParticipantEmail,
                        isGuest: ctx.user.isGuest,
                    },
                });
            }
    ...
    getWatchers: publicProcedure
        ...
        .query(async ({ input: { pollId } }) => {
            return await db.watcher.findMany({
                where: {
                    pollId,
                },
                select: {
    ...
    book: proProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const scheduledEvent = await db.$transaction(async (tx) => {
                ...
                for (const p of participantsToEmail) {
                    ...
                    getEmailClient(p.locale ?? undefined).queueTemplate(
                        'FinalizeParticipantEmail',
                        {
                            ...
                            attachments: [
                                { filename: 'event.ics', content: event.value },
                            ],
                        }
                    );
                }
