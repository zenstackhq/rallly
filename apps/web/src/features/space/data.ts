...
function createMemberDTO(member: {
    ...
    userId: string;
    spaceId: string;
    role: ModelTypes.SpaceMemberRole;
    space: {
        ownerId: string;
    };
    user: {
...
export async function getSpaceSeatCount(spaceId: string) {
    return await db.spaceMember.count({
        where: {
            spaceId: spaceId,
        },
    });
...
export const loadInvites = cache(async () => {
    ...
    const invites = await db.spaceMemberInvite.findMany({
        where: {
            spaceId: space.id,
        },
        include: {
