...
export const setActiveSpaceAction = authActionClient
    ...
    .action(async ({ ctx, parsedInput }) => {
        const space = await db.space.findFirst({
            where: {
                AND: [
                    accessibleBy(ctx.ability).Space,
    ...
    .action(async ({ ctx, parsedInput }) => {
        ...
        if (ctx.getMemberAbility().cannot('create', 'SpaceMemberInvite')) {
            throw new AppError({
                code: 'FORBIDDEN',
                message:
                    'You do not have permission to invite members to this space',
            });
        }
        ...
        const existingInvite = await db.spaceMemberInvite.findUnique({
            where: {
                spaceId_email: {
        ...
        if (existingInvite) {
            ...
            if (existingInvite.role !== toDBRole(parsedInput.role)) {
                await db.spaceMemberInvite.update({
        ...
        try {
            const invite = await db.spaceMemberInvite.create({
                data: {
                    spaceId: ctx.space.id,
                    email: parsedInput.email,
...
export const acceptInviteAction = authActionClient
    ...
    .action(async ({ ctx, parsedInput }) => {
        const spaceMemberInvite = await db.spaceMemberInvite.findUnique({
            where: {
                spaceId_email: {
                    spaceId: parsedInput.spaceId,
        ...
        const { memberCount } = await db.$transaction(async (tx) => {
            ...
            const [usedSeats, totalSeats] = await Promise.all([
                tx.spaceMember.count({
                    where: { spaceId: parsedInput.spaceId },
                }),
...
export const removeMemberAction = spaceActionClient
    ...
    .action(async ({ ctx, parsedInput }) => {
        ...
        const deletedMember = await db.spaceMember.delete({
            where: {
                id: parsedInput.memberId,
            },
        });

        const memberCount = await db.spaceMember.count({
            where: {
...
export const changeMemberRoleAction = spaceActionClient
    ...
    .action(async ({ ctx, parsedInput }) => {
        ...
        if (
            ctx
                .getMemberAbility()
                .cannot('update', subject('SpaceMember', member))
        ) {
            throw new AppError({
                code: 'FORBIDDEN',
                message:
...
export const cancelInviteAction = spaceActionClient
    ...
    .action(async ({ ctx, parsedInput }) => {
        ...
        await db.spaceMemberInvite.delete({
            where: { id: parsedInput.inviteId },
        });
    });
...
export const leaveSpaceFromAccountAction = authActionClient
    ...
    .action(async ({ ctx, parsedInput }) => {
        ...
        const userSpaces = await db.spaceMember.findMany({
            where: {
                userId: ctx.user.id,
                NOT: {
                    spaceId: parsedInput.spaceId,
