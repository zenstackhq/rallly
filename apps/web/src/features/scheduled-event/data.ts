...
import { db, type InputTypes, type ModelTypes } from '@rallly/database';
...
function getEventsWhereInput({
    ...
    const where: InputTypes.ScheduledEventWhereInput = {
        spaceId,
        deletedAt: null,
        ...(status === 'upcoming' && {
            OR: [
...
export const getScheduledEvents = async ({
    ...
    const [rawEvents, totalCount] = await db.$transaction([
...
export const getUpcomingEventsCount = async () => {
    const space = await requireSpace();
    return db.scheduledEvent.count({
...
function buildBaseWhere(
    ...
    search?: string,
    member?: string
): InputTypes.ScheduledEventWhereInput {
    const baseWhere: InputTypes.ScheduledEventWhereInput = {
        spaceId,
        deletedAt: null,
    };
...
export const getUpcomingEvents = async ({
    ...
    const [totalCount, allEvents] = await db.$transaction([
        db.scheduledEvent.count({ where }),
        db.scheduledEvent.findMany({
            where,
