...
import { db, ModelTypes } from '@rallly/database';
...
async function createScheduledEventForUser({
    ...
    const status = faker.helpers.arrayElement<ModelTypes.ScheduledEventStatus>([
        ModelTypes.ScheduledEventStatus.confirmed,
        ModelTypes.ScheduledEventStatus.canceled,
    ]);
        ...
        data: {
            ...
            invites: {
                create: Array.from({ length: randInt(5, 0) }).map(() => ({
                    ...
                    status: faker.helpers.arrayElement<ModelTypes.ScheduledEventInviteStatus>(
...
async function seedTeamEvents() {
    ...
    const teamSpace = await db.space.findUnique({
